...
def extract_fields(bound_min, bound_max, resolution, func):
    N = 32
        ...
        for xi, xs in enumerate(X):
            for yi, ys in enumerate(Y):
                for zi, zs in enumerate(Z):
                    ...
                    val = func(pts).reshape(len(xs), len(ys), len(zs)).detach().cpu().numpy()
...
def extract_mesh(func, bbox_min, bbox_max, resolution=256, threshold=0.0, points_gt=None, mesh_far=0.0):
